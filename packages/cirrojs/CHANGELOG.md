...
### Added
- Configuration validation at startup: `cssUrl` must be a root-relative path, the `routes` and `islands` files must exist (all reported as errors), and a missing `watchDir` is reported as a warning (file watching simply does not fire).
- Route validation after route expansion, shared by dev and build: malformed output paths (missing leading `/`, `..`/`.` segments, backslashes, `?`/`#`, control characters, html routes not ending in `.html`/`.htm`, file routes without an extension), duplicate output paths (e.g. duplicate slugs from `getStaticPaths`), and collisions between route outputs and `public/` files on any served spelling (dev and build could otherwise pick different winners). The dev server warns; `cirro build` reports every violation and exits with a non-zero code.
- Island usage validation during rendering: rendering `<Island>` when the `islands` option is not configured (nothing would hydrate), or with a name that is not a key of the configured registry (the client mounter silently skips it), is collected and reported with the page it occurred on. Loading the registry also validates that it default-exports an object, so a missing default export now fails in dev too instead of only logging to the browser console.
- Island props serialization validation: every `<Island>` props value must survive a JSON round-trip. Functions, symbols, `undefined`, bigints, non-finite numbers, non-plain objects (`Date`, `Map`, React elements, …), and circular references are reported with the property path; such values are silently dropped or altered by `JSON.stringify` and break hydration.
- `cirro build` warns when a `@layer` block uses a layer name never declared in a `@layer a, b, ...` statement (`defineCascadeLayer()`), since layer priority then falls back to first-use order.
- `registerIslandUsage(name, props)`, exported from `cirrojs/registry` (called by `createIsland`; the browser build resolves it to a no-op), and the `IslandError` / `IslandPropsError` types. `RenderSiteContext` gained an optional `islandNames` field carrying the configured registry keys.
...
### Changed
- The `react()` plugin may now be registered before or after `cirro()`; the previous "must come before" requirement was verified to be unnecessary (cirro performs no JSX transform). Registering it first is still the documented convention.
