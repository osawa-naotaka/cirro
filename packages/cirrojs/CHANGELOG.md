...
- Markdown reference validation: `a href` and `img src` in Markdown bodies are validated at render time against the site's URL set (all routes plus `public/` files), riding the same classification as `<Link>`. Root-relative references are checked for existence, `#` anchors and scheme-prefixed URLs (external links and images) pass through, and relative or protocol-relative paths are reported as malformed. Violations carry the attribute (`href`/`src`) so they can be distinguished from JSX-originated ones. The check runs in the trusted layer after `rehype-sanitize`, so links generated by user plugins are also validated. It is enabled by default and can be disabled with the new `MarkdownConfig.checkRefs: false`; rendering outside of a page render context (e.g. inside a content loader) skips validation instead of throwing.
- Configuration validation at startup: `cssUrl` must be a root-relative path and the `routes` / `islands` files must exist (all reported as errors); a missing `watchDir` is reported as a warning (file watching simply does not fire).
...
- New exports from `cirrojs/registry`: `registerIslandUsage(name, props)` (called by `createIsland`), `checkMarkdownRef(attr, ref)` (called by the Markdown pipeline), `requireSite(feature)`, `reportMissingSiteConfig(feature)`, `currentPagePath()`, and `htmlPagePaths()`. The browser build resolves the register/check functions to no-ops and the site helpers to explicit server-only errors. New types: `ErrorInfo`, `MissingSite`, `IslandError`, `IslandPropsError`, `MarkdownRef`, and `RenderSiteContext`.
...
- **Breaking** (`cirrojs/registry` consumers only): the `runWithRegistry` result consolidates the former `brokenLinks` / `brokenImageSrc` arrays into a single `errors: ErrorInfo[]`, and the `BrokenLink` / `BrokenImageSrc` types gained a `cause` discriminator (`"broken-link"` / `"broken-image-src"`). `runWithRegistry` also accepts an optional fourth argument carrying the render context (site metadata, current page path, html page list, configured island names). File routes are now rendered inside the registry context in both dev and build, so `<Link>`-style validation (e.g. RSS item paths) works and is reported for file routes as well.
