...
### Added
- Markdown reference validation: `a href` and `img src` in Markdown bodies are validated at render time against the site's URL set (all routes plus `public/` files), riding the same classification as `<Link>`. Root-relative references are checked for existence, `#` anchors and scheme-prefixed URLs (external links and images) pass through, and relative or protocol-relative paths are reported as malformed. Violations carry the attribute (`href`/`src`) so they can be distinguished from JSX-originated ones; the dev server warns and `cirro build` reports them per page and exits non-zero. The check runs in the trusted layer after `rehype-sanitize`, so links generated by user plugins are also validated. It is enabled by default and can be disabled with the new `MarkdownConfig.checkRefs: false`; rendering outside of a page render context (e.g. inside a content loader) skips validation instead of throwing.
- `checkMarkdownRef(attr, ref)` and the `MarkdownRef` type, exported from `cirrojs/registry` (browser build is a no-op).
