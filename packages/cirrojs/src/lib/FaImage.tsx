...
export function FaImage({ icon, ...props }: FaImageProps): ReactNode {
    ...
    const path = require.resolve(`@fortawesome/fontawesome-free/svgs/${icon.type}/${icon.name}.svg`);
    const svg = readFileSync(path, "utf-8");
    const match = svg.match(/<svg[^>]*viewBox="(?<viewBox>[0-9 ]+)"[^>]*>(?<content>.+)<\/svg>/);

    const viewBox = match?.groups?.viewBox;
    const content = match?.groups?.content;

    return <svg xmlns="http://www.w3.org/2000/svg" viewBox={viewBox} fill="currentColor" {...props} dangerouslySetInnerHTML={{ __html: content }} />
}
