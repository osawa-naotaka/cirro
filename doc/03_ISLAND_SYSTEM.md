...
「本文（静的部分）のコード・データをクライアントへ送らない」を、技術的にどう実現しているかを
記録する。
...
- **島のスタイルは初期 SSR 描画で全て登録すること**: 自前 CSS（`css()`）は **SSR 描画で実際に実行された
  分だけ**を生成する。初期状態で描画されない分岐（`{open && <Panel/>}` など）に閉じた `css()` は CSS が
  未生成になり、クライアントでその分岐が現れると無スタイルで表示される。遅延マウントされる部分は
  `styleSample()` でサンプル要素を登録する（`Disclosure` 島が手本）か、全バリアントを無条件に評価して
  `className` の差し替えで切り替える（`ScrollTop` 島が手本）、条件付きマウントより CSS による表示/非表示を
  優先する、等で回避する。詳細と理由は `05_STYLING.md` 7.3 を参照。
