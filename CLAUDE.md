...
**手順**:
1. packages/cirrojs/CHANGELOG.mdに今回リリース分の変更は一部がUnreleasedに記録されているものとする。
2. packages/cirrojs/CHANGELOG.mdを更新する
...
3. packages/cirrojs/package.jsonを確認し、パッケージバージョンが最新であることを確認する。最新でない場合は最新に変更する。
4. packages/create-cirrojs/package.jsonを確認し、パッケージバージョンがcirrojsと一致していることを確認する。一致していない場合は一致するように更新する。
5. packages/create-cirro/template/_package.json の `dependencies.cirrojs` を、今回リリースする cirrojs のバージョン（キャレット指定。例 `^0.0.30`）に更新する。スキャフォールディングのテンプレートは公開された cirrojs に依存するため、リリースのたびにこの追従が必要（`doc/16_SCAFFOLDING.md` 4.1 参照）。
6. ワーキングブランチにコミットする
